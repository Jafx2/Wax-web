...
import { Palette, Star, ListMusic, Music2 } from 'lucide-react'
...
function TrackPlayer({ track }) {
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #1a1508, #111)', border: '1px solid rgba(232,197,71,0.15)', borderRadius: 14, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14 }}>
      <audio ref={audioRef} />
      <img src={track.image} alt={track.name || 'Canción'} style={{ width: 48, height: 48, borderRadius: 8, objectFit: 'cover', flexShrink: 0 }} referrerPolicy="no-referrer" />
      <div style={{ flex: 1, minWidth: 0 }}>
...
function AlbumCover({ albumId, number }) {
  ...
  useEffect(() => {
    if (!albumId) return
    supabase.from('albums').select('album_id, title, artist, cover_url').eq('album_id', albumId).single().then(({ data }) => {
...
export default function ProfileClient({ usernameParam }) {
  ...
  async function loadProfile() {
    ...
    if (likeRows?.length) {
      const [{ data: likedTextPosts }, { data: likedReviews }] = await Promise.all([
        supabase.from('posts').select('id, body, created_at, profiles(username, display_name, avatar_url), albums(title, artist, cover_url)').in('id', likeRows.map(r => r.post_id)),
        supabase.from('reviews').select('id, body, rating, created_at, profiles!reviews_user_id_fkey(username, display_name, avatar_url), albums!reviews_album_id_fkey(title, artist, cover_url)').in('id', likeRows.map(r => r.post_id)),
      ])
    ...
    if (respinRows?.length) {
      const [{ data: respinTextPosts }, { data: respinReviews }] = await Promise.all([
        supabase.from('posts').select('id, body, created_at, profiles(username, display_name, avatar_url), albums(title, artist, cover_url)').in('id', respinRows.map(r => r.post_id)),
        supabase.from('reviews').select('id, body, rating, created_at, profiles!reviews_user_id_fkey(username, display_name, avatar_url), albums!reviews_album_id_fkey(title, artist, cover_url)').in('id', respinRows.map(r => r.post_id)),
      ])
    ...
    const [{ count: followers }, { count: following }, { count: ratingsCount }] = await Promise.all([
      ...
      supabase.from('reviews').select('*', { count: 'exact', head: true }).eq('user_id', prof.id),
    ])
    ...
    if (listsRes.ok) {
      ...
      setFeaturedLists((listsData.lists || []).filter(l => l.featured))
    }
        ...
        <div className="profile-banner-inner" style={{ maxWidth: 900, margin: '0 auto', padding: '0 48px', position: 'relative' }}>
          {isOwn && (
            <div style={{ position: 'absolute', top: 12, right: 48, zIndex: 5 }}>
              ...
              >
                <Palette size={13} /> Color del banner
              </button>
              {showBannerPicker && (
                <div style={{

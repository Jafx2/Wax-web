...
function CreatePost({ currentUser, profile, onPost, prefillAlbum }) {
    ...
    const handleSubmit = async () => {
        ...
        const res = await authFetch('/api/posts', {
            ...
            body: JSON.stringify({ body: body.trim(), albumId: album?.id || null, type: 'text' }),
        })
...
// ── MAIN CLIENT ───────────────────────────────────────────
export default function FeedClient({ initialPosts, topAlbums }) {
    ...
    async function loadPosts() {
        ...
        const normalized = (data || []).map((post) => ({
            ...
            respin_count: post.respin_count || post.respins?.length || 0,
        }))
        setPosts(normalized)
        setLoading(false)
    }
    ...
    const handleRespin = async (post) => {
        ...
        const payload = await res.json()
        if (payload?.post) {
            setPosts(prev => prev.map(p => p.id === post.id ? { ...p, respin_count: payload.post.respin_count || 0 } : p))
        }
    }
    ...
    return (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                {topAlbums.map((album, i) => (
                                    <Link key={album.album_id} href={`/album/${album.album_id}`}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 6px', borderRadius: 8, transition: 'background 0.15s' }}
        ...
        @media (max-width: 768px) {
          ...
          .post-card { padding: 20px 0 !important; }
          .post-review-block { gap: 14px !important; }
          .post-cover { width: 84px !important; height: 84px !important; }
          .post-album-title { font-size: 18px !important; }
          .post-body { font-size: 13.5px !important; line-height: 1.55 !important; margin-bottom: 14px !important; }
          .post-avg-rating { font-size: 11px !important; }
          .post-actions { gap: 16px !important; }
        }
        ...
        .like-heart-pop { animation: likeHeartPop 0.4s ease; }
